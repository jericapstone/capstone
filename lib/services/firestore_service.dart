...
// Delete a room from the 'MTROOMS' collection
  ...
  Future<List<Room>> fetchRooms() async {
    try {
      final snapshot = await roomsCollection.get(); // Get all documents from 'rooms' collection
      return snapshot.docs.map((doc) {
        return Room.fromFirestore(doc); // Convert document to Room object
      }).toList();
    } catch (e) {
      print('Error fetching rooms: $e');
      return [];
  ...
  Future<void> addRoom(Room room) async {
    try {
      await roomsCollection.doc(room.id).set(room.toMap()); // Add room using its ID
    } catch (e) {
      print('Error adding room: $e');
    }
  }
  ...
  Future<void> updateRoom(Room room) async {
    try {
      await roomsCollection.doc(room.id).update(room.toMap()); // Update the room based on ID
    } catch (e) {
      print('Error updating room: $e');
    }
  }
  ...
  Future<void> deleteRoom(String id) async {
    try {
      await roomsCollection.doc(id).delete(); // Delete the room based on ID
    } catch (e) {
      print('Error deleting room: $e');
    }
  }
